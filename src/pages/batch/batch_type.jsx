...
const BatchType = () => {
    ...
    const getBatchType = React.useCallback(() => {
        http.get(`/api/batch/type`).then((res) => {
    ...
    const onSubmit = (event) => {
        ...
        if (form?._id) {
            http.put(`/api/batch/type/${form?._id}`, data).then((res) => {
                getBatchType();
                dispatch(OPEN_SNACKBAR({ message: 'Batch Type Update Successfully.' }));
                handleClose();
            })
        } else {
            http.post('/api/batch/type', data).then((res) => {
                getBatchType();
                dispatch(OPEN_SNACKBAR({ message: 'New Batch Type Added.' }));
                handleClose();
            })
        }
    };
    ...
    return <React.Fragment>
        <TableComponent columns={columnsData} data={rowData} tableHooks={tableHooks} topComponent={
            <React.Fragment>
                <Tooltip title="Add New Batch Type">
                    <Button variant="contained" endIcon={<AddIcon />} onClick={handleClickOpen}>
                        <Hidden only={'xs'}>Add Batch Type</Hidden>
                    </Button>
                </Tooltip>
            </React.Fragment>
        } />
        <Dialog open={open} fullWidth maxWidth={'sm'}>
            <Box component="form" onSubmit={onSubmit} noValidate autoComplete="off">
                <DialogTitle>{form?._id ? 'Edit' : 'Add New'} Batch Type</DialogTitle>
                <DialogContent>
                    <TextField
                        autoFocus
                        margin="dense"
